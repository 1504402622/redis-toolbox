...
func (f *Filter) Allow(cmd client.Command) bool {
	...
	if _, ok := f.blockDB[cmd.DB]; ok {
		f.stats.blocked++
		log.Infof("command blocked by DB filter: %s (db=%d)", cmd.Name, cmd.DB)
		return false
	}
	...
	if _, ok := f.blockCmd[cmd.Name]; ok {
		f.stats.blocked++
		log.Infof("command blocked by command filter: %s", cmd.Name)
		return false
	}
	...
	if cmd.Key != "" {
		...
		if _, ok := f.blockKeyExact[cmd.Key]; ok {
			f.stats.blocked++
			log.Infof("command blocked by exact key match: %s (key=%s)", cmd.Name, cmd.Key)
			return false
		}
		...
		for _, p := range f.blockKeyPrefix {
			if strings.HasPrefix(cmd.Key, p) {
				f.stats.blocked++
				log.Infof("command blocked by key prefix filter: %s (key=%s, prefix=%s)", cmd.Name, cmd.Key, p)
				return false
			}
		}
